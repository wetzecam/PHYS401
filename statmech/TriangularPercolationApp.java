...
public class TriangularPercolationApp extends AbstractCalculation implements InteractiveMouseHandler {
  LatticeFrame lattice = new LatticeFrame("Percolation");
  Random random = new Random();
  int L;
  int clusterNumber;  // used to color clusters

  public TriangularPercolationApp(){
  ...
  public void calculate() {
    ...
    // j=0 (left) j=1(right)  j=2(down) j=3(up),  j=4 (down-left), j=5 (uo-right)
    ...
    public static void main(String[] args){
      CalculationControl control = CalculationControl.createApp(new TriangularPercolationApp());
    }
}
