...
public class ThreePI extends AbstractCalculation{
  ...
  double Local_Energy_Sum(double[] bead, double dt){
    ...
    for(int i=0;i<(bead.length-1);i++){
      E = E + local_Energy(bead[i],bead[i+1],dt);
    }
  ...
  double[] MC_Path_Integral_v(double Time, int N_beads, int nmcs){
    ...
    double dt = Time/((double)(bead.length));
    ...
    for(int i=0;i<nmcs;i++){
      ...
      for(int XX=0; XX<100;XX++){
        ...
        for(int j=0;j<bead.length;j++){
          ...
          N_trial++;
          //int J = j;//(int)Math.random()*(bead.length);
      ...
      double EL = Local_Energy_Sum(bead,dt)/N_beads;
      eAccum += EL;
      eAccum2 += EL*EL;
    }
    System.out.println("Acceptance Rate = "+((double)(N_accepted)/(double)(N_trial)));
    ...
    double var_X = Math.sqrt((eAccum2)/((double)(nmcs)) - out[0]*out[0]);
  ...
  public void calculate(){
    try{
      FileWriter ofile = new FileWriter("ThreePIdata2_new.txt");
    ...
    try{
      FileWriter ofile = new FileWriter("ThreePIdata4_new.txt");
      BufferedWriter oss = new BufferedWriter(ofile);
    ...
    try{
      FileWriter ofile = new FileWriter("ThreePIdata8_new.txt");
      BufferedWriter oss = new BufferedWriter(ofile);
    ...
    /*
    try{
      FileWriter ofile = new FileWriter("ThreePIdata16_new.txt");
      BufferedWriter oss = new BufferedWriter(ofile);

      double Time = 1.0;
      while(Time < TMAX){
        double[] E_avg = MC_Path_Integral_v(Time, 16, NMC);
        System.out.println("Time = "+Time+"  E = "+E_avg[0]);
        oss.write(Time+" "+E_avg[0]+" "+E_avg[1]+" "+Anal_Solution(Time));
        oss.newLine();
        Time += 0.5;
  ...
  public static void main(String[] args){
    CalculationControl control = CalculationControl.createApp(new ThreePI());
  }
