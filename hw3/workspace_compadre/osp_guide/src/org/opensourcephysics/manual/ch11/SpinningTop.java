...
public class SpinningTop extends RigidBody {
  ...
  void computeBodyFrameTorque(double[] state) {
    quaternion.setCoordinates(state[0], state[2], state[4], state[6]);
    double[] vec = quaternion.inverse(new double[] {0, 0, 1}); // transforms gravitational torque from space to body frame
